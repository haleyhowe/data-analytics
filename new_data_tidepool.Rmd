...
```{r}
#Frequency of low event and user checking fingerstick within 5 minutes of eachother 
...
count_2=0
  for (i in 1:nrow(smbg_month)){
    ...
    cat("\nFrequency of BG Check entry within *5 min* of Low glucose alert: ",count_2)
...
#Low glucose alerts and insulin bolus times 
  ...
  for (i in 1:nrow(bolus_month)){
   ...
   if(count_3 ==0){
     cat("\nThere were no events of a low glucose alert and insulin bolus that have occured within 60 min of each other.")
   }
...
## Frequency of insulin bolus within 1 min of low BG alert
 ...
 cat("\nFrequency of insulin bolus within *1 min* of Low BG alert:", count_4)
...
## Frequency of insulin bolus within 1 min of high BG alert
  ...
  cat("\nFrequency of insulin bolus within *1 min* of high BG alert: ", count_5)
  ...
  for (i in 1:nrow(smbg_month)){
    for (x in 1:nrow(cbg_month)){
      ...
      if(strptime(smbg_month$time[i], format = "%Y-%m-%d %H:%M:%S", tz="UTC") > strptime(cbg_month$time[x], format = "%Y-%m-%d %H:%M:%S", tz="UTC")){
        if((strptime(smbg_month$time[i], format = "%Y-%m-%d %H:%M:%S", tz="UTC")- strptime(cbg_month$time[x], format = "%Y-%m-%d %H:%M:%S", tz="UTC")) < dminutes(5)){
        count_7 = count_7 + 1
        cat(strptime(smbg_month$time[i], format = "%Y-%m-%d %H:%M:%S", tz="UTC")- strptime(cbg_month$time[x], format = "%Y-%m-%d %H:%M:%S", tz="UTC"), "\n\nFingerstick Input Time:", smbg_month$time[i], "\nHigh Glucose Alert Time:", cbg_month$time[x])
...
cat("US-KIDS Survey Data Statistics")
cat( "\nTotal number of high alerts for the day: ", high_events, "\nTotal number of low alerts for the day: ", low_events)

cat("\nDate: ",  month(strptime(cbg_month$time[1], format = "%Y-%m-%d %H:%M:%S", tz="UTC")) ,"/", day(strptime(cbg_month$time[1], format = "%Y-%m-%d %H:%M:%S", tz="UTC")))

cat("\nLOW ALERTS")
if(low_events!= 0){
cat("\nPercentage of time user recieved a low alert and checked fingerstick value within *30 minutes* of each other: ",percent(count_1/low_events), count_1)


cat("\nPercentage of time user recieved a low alert and checked fingerstick value within *5 minutes* of each other: ", percent(count_2/low_events))

cat("\nPercentage of time user recieved a low alert and recieved an insulin bolus within *60 minutes* of each other: ", percent(count_3/low_events))

cat("\nPercentage of time user recieved an insulin bolus within *1 min* of Low BG alert: ",  percent(count_4/low_events) ) 
}

if(low_events == 0){
cat("\nThere were no low alerts for this day")
}
cat("\nHIGH ALERTS")
...
cat("\nPercentage of time user recieved a high alert and checked fingerstick value within *30 minutes* of each other: ",percent(count_6/high_events))

cat("\nPercentage of time user recieved a high alert and checked fingerstick value within *5 minutes* of each other: ", percent(count_7/high_events))
}
if(high_events == 0){
cat("\nThere were no high alerts for this day")
}
```
